---
layout: post
title: "On class dynamics in Zimbabwe"
date: 2025-10-18 20:45:00
file: https://github.com/valentinegarikayi/podcasts/raw/master/audio/Ep_19_2025_BMoyo_October.mp3
summary: "On the importance of understanding class relations in Zimbabwe"
description: "A conversation with Comrade Benny Moyo"
duration: "01:03:38"
length: "48034620"
explicit: "no"
keywords: "Class Culture"
block: "no"
transcript: /transcripts/ep-19/
thumbnail: /images/zim-fighter.jpg
---

Since independence in 1980, Zimbabwe’s ruling ZANU–PF has shaped a society marked by deepening class divides. Outside the political elite, a wealthy class has risen. Examples include figures like Strive Masiyiwa and Kudakwashe Tagwirei. At the same time, the once-stable professional class of teachers, nurses, and clerks has steadily declined. Today, a small middle class survives alongside peasants, vendors, and artisanal miners facing harsh realities.

In this episode, I sit down with Comrade Benny Moyo to explore class in Zimbabwe from the rise of the elite to the search for a revolutionary class.

Outro Music: "Famba So" by Simon Chimbetu
<!--more-->

## Important links

* [The Rise of an African Middle Class: Colonial Zimbabwe, 1898-1965](https://sahistory.org.za/sites/default/files/archive-files/michael_o._west_the_rise_of_an_african_middle_clbook4you.pdf)
* [Jason Ziyaphapha Moyo](https://www.colonialrelic.com/biographies/106-2/)

![Elections]({{ site.url }}{{ site.baseurl }}/images/Wicknell-Chivayo-Cash.webp){:width="300"}

<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-02DTBF3N7T"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-02DTBF3N7T');
</script>
